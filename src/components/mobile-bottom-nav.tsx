...
export function MobileBottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 z-50 w-full">
      <nav className="bg-card/95 backdrop-blur-lg border-t border-primary/20 shadow-[0_-8px_30px_rgb(0,0,0,0.12)] rounded-t-[2.5rem] h-24 flex items-center justify-around px-4 pb-4">
        {mainItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex flex-col items-center justify-center gap-1.5 text-muted-foreground active:scale-90 transition-all outline-none flex-1">
              ...
              <span className="text-[12px] font-black uppercase tracking-tight">Mais</span>
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent 
            align="end" 
            ...
            {moreItems.map((item) => (
              <DropdownMenuItem key={item.href} asChild>
                <Link
                  href={item.href}
                  className={cn(
                    "flex items-center gap-4 px-5 py-5 rounded-2xl cursor-pointer group mb-1 last:mb-0 transition-colors",
